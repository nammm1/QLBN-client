...
const NutritionistWorkSchedule = () => {
  ...
  const fetchAppointments = async (date, ca) => {
    try {
      ...
      const res = await apiCuocHenTuVan.getByDateAndCa(dateStr, ca);
          ...
          <List.Item.Meta
            ...
            description={
              <Space direction="vertical" size={0} style={{ fontSize: '12px' }}>
                ...
                <Text><MedicineBoxOutlined /> {appt.ly_do_tu_van || appt.ly_do_kham}</Text>
            ...
            <Col>
              <Space direction="vertical" size={0}>
                ...
                <Text style={{ 
                  ...
                  Chuyên gia dinh dưỡng: <Text strong style={{ color: 'white' }}>{userInfo?.user?.ho_ten || ""}</Text>
                </Text>
              </Space>
            </Col>
            <Col>
...
export default NutritionistWorkSchedule;